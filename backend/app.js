...
const CLIENT_ID =
  "791042030465-vt4ltp51qct40th4uo4i9b3da3dsccd4.apps.googleusercontent.com";
const CLEINT_SECRET = "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";
const REDIRECT_URI = "https://developers.google.com/oauthplayground";
const REFRESH_TOKEN =
  "1//04-8ZFEC0oq-0CgYIARAAGAQSNwF-L9IrpuFA1TDqcPAmVCRUSjgZkV6PFSuGk-9IFzdVKxEOn9UeyvGwK2BPquQW1KplFyxeaJg";
...
const sentmail = async function sendMail(options) {
  try {
...
userSchema.methods.getJWTtoken = function () {
  return jwt.sign({ id: this._id }, process.env.JWTSECRETE, {
...
sellerSchema.methods.comparePassword = async function (getpassword) {
  return await bcrypt.compare(getpassword, this.password);
};
...
app.post("/registercomplaint", isAUthenticatedUser, async function (req, res) {
  console.log("Body sending....", req.body);
  console.log("Body sending....", req.user.id);
  const existuser = await User.findById(req.user.id);
  console.log(existuser);
  const existComplain = await Complain.findOne({
    order_id: req.body.order_id,
  });
  console.log("Query Result.... ", existComplain);
  const existOrder = await Order.findOne({ order_id: req.body.order_id });
  if (
    req.body.subject === "" &&
    ...
    return res.status(401).json({
      success: false,
      message: "you are entering wrong order ID",
    });
  } else if (existComplain) {
    return res.status(401).json({
      success: false,
      message: "the complain on this order already registered",
    });
  } else if (existuser.email !== existOrder.customer) {
    return res.status(401).json({
      success: false,
      message: "this product is not ordered by your account",
    });
  } else {
    const complain = new Complain({
      ...
      customer_email: existOrder.customer,
      seller_email: existOrder.seller,
    });
    complain.save((err, data) => {
      ...
      if (err) throw err;
        ...
        res
          .status(200)
          .json({ message: "your complain is successfully registered" });
      }
    });
  }
});

//show complain to customer

app.get("/getcomplaint", isAUthenticatedUser, async function (req, res) {
  try {
    console.log("Body sending....", req.user.id);
    const user = await User.findById({ _id: req.user.id });
    console.log(user);
    const complain = await Complain.find({ customer_email: user.email });
...
app.get(
  ...
  async function (req, res) {
    try {
      console.log("Body sending....", req.user.id);
      const seller = await Seller.findById({ _id: req.user.id });
      console.log(seller);
      const complain = await Complain.find({ seller_email: seller.emailID });
      console.log("Complain:", complain);
      if (complain.length === 0) {
        return res.send({ message: "there is no complain to view" });
      } else {
        return res.send(complain);
      }
    } catch (error) {
...
// show to complain to admin

app.get("/getadmincomplaint", async function (req, res) {
  try {
...
//updateStatus of complain

app.put("/updateadmincomplaint/:id", async function (req, res) {
  try {
    ...
    console.log("Complain:", complain);
    if (complain.length === 0) {
      return res.send({ message: "there is no complain" });
    } else {
      return res.send({ message: "status change sucessfully" });
    }
  } catch (error) {
    console.error(error.message);
    res.status(500).send("Internal Server Error");
...
//delete the complain
app.delete("/deladmincomplaint/:id", async function (req, res) {
  try {
    const complain = await Complain.findByIdAndRemove({ _id: req.params.id });
    console.log("Complain:", complain);
    if (complain.length === 0) {
      return res.send({ message: "there is no complain to delete" });
    } else {
...
app.post("/loginCustomer", async function (req, res) {
  ...
  const isPasswordMatch = await existUser.comparePassword(req.body.password1);
  if (isPasswordMatch) {
...
app.post("/userforget", async function (req, res) {
  ...
  const resetPasswordUrl = `http://localhost:3000/userpassword/reset/${resetToken}`;
...
app.get("/userpassword/reset/:token", async (req, res) => {
  try {
...
app.post("/userpassword/reset/:token", async function (req, res) {
  // creating token hash
...
//adminForget
app.post("/adminforget", async function (req, res) {
  const user = await User.findOne({ email: req.body.email1 });
  console.log(user);
...
app.post("/sellerforget", async function (req, res) {
  const user = await Seller.findOne({ emailID: req.body.email1 });
  console.log(user);
  ...
  const resetPasswordUrl = `http://localhost:3000/sellerpassword/reset/${resetToken}`;
...
app.get("/sellerpassword/reset/:token", async (req, res) => {
  try {
    const token = await User.findOne({
      resetPasswordtoken: req.params.token,
    });
...
app.post("/sellerpassword/reset/:token", async function (req, res) {
  // creating token hash
  const resetPasswordtoken = crypto
    .createHash("sha256")
    .update(req.params.token)
...
app.post("/search", async function (req, res) {
  ...
  const foundProduct = await Product.find({
    $or: [
      ...
      { category: { $regex: req.body.name, $options: "i" } },
    ],
  });
...
app.get("/getSuggestions", async function (req, res) {
  ...
  res.send(products[0].top_Products);
...
app.get("/getSuggestions/:userName", async function (req, res) {
  ...
  //const topProducts = products[0].top_Products;
...
app.get(`/getBrandProducts/:brand`, async (req, res) => {
...
app.get(`/getRamProducts/:ram`, async (req, res) => {
  console.log(req.params);
  const products = await Product.find({ ram: req.params.ram });
  if (products) {
    res.send(products);
  } else {
    res.send({
...
app.get(`/getPriceProducts/:max/:min/:name`, async (req, res) => {
  console.log(req.params);
  ...
  products = products.filter(
    (element) =>
      element.name === req.params.name || element.category === req.params.name
  );
  console.log(products);
  if (products) {
    res.send(products);
...
app.post("/loginAdmin", async function (req, res) {
  ...
  const isPasswordMatch = await existUser.comparePassword(req.body.password);
  console.log(isPasswordMatch);
  if (isPasswordMatch) {
    console.log("Found user....", existUser);
...
// daraz webscraping
app.post("/Daraz", async function (req, res) {
  const pythonChild = await spawn("python", ["Daraz.py", req.body.name]);
  const products = [];
